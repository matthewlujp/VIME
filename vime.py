...
class BNN:
    ...
    def _linear(W_mu, b_mu, W_var, b_var, X):
        ...
        """
        print("X", X.size(), "W_mu", W_mu.size(), "b_mu", b_mu.size())
        gamma = X @ W_mu + b_mu
        delta = X @ W_var
    ...
    def calc_log_likelihood(self, batch_s_next, batch_s, batch_a):
        ...
        l = - .5 * (s_next_logvar + (batch_s_next - s_next_mean).pow(2) * (- s_next_logvar).exp()).sum(dim=1) - len(s_next_mean)/2 * np.log(2 * np.pi)
